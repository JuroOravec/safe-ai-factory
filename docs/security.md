...
`safe-ai-factory` (SAIF) is built around a **zero-trust** architecture. The coding agent's container is considered unsafe. Any code it generates is considered unsafe.

SAIF enforces multiple layers of security to ensure the agent physically cannot cheat, reward-hack, or break out of its sandbox.
...
If the agent can see the tests, it will write code that perfectly hardcodes the expected responses, bypassing the actual implementation. SAIF prevents this entirely:
