...
```bash
saif feat run --push owner/repo --pr --git-provider github
```
...
```bash
export GITEA_TOKEN=...
saif feat run --push origin --pr --git-provider gitea
```
...
```bash
saif feat run --push origin --pr --git-provider ./my-gitea-provider.js
```
