...
set -eu
cd /workspace
echo "[sandbox/python-pip-node/startup] Installing Python dependencies (pip)..."
if command -v uv > /dev/null 2>&1 && [ -f pyproject.toml ]; then
  uv sync
elif [ -f requirements.txt ]; then
  pip install -r requirements.txt
else
  echo "[sandbox/python-pip-node/startup] No requirements.txt or pyproject.toml found — skipping."
fi
echo "[sandbox/python-pip-node/startup] Done."
