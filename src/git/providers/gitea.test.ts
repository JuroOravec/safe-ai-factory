...
describe('GiteaProvider.resolvePushUrl', () => {
  ...
  it('injects GITEA_USERNAME and GITEA_TOKEN into a gitea.com HTTPS URL', () => {
    process.env.GITEA_TOKEN = 'mytoken';
    process.env.GITEA_USERNAME = 'myuser';
    const p = makeProvider();
    const result = p.resolvePushUrl('https://gitea.com/owner/repo.git', FAKE_ROOT);
    expect(result).toContain('myuser:mytoken@gitea.com');
  });

  it('injects credentials into a self-hosted Gitea HTTPS URL when GITEA_URL matches', () => {
    process.env.GITEA_TOKEN = 'mytoken';
    process.env.GITEA_USERNAME = 'myuser';
    process.env.GITEA_URL = 'https://gitea.mycompany.com';
    const p = makeProvider();
    const result = p.resolvePushUrl('https://gitea.mycompany.com/owner/repo.git', FAKE_ROOT);
    expect(result).toContain('myuser:mytoken@gitea.mycompany.com');
  });

  it('does NOT inject credentials into a non-configured HTTPS host (credential safety)', () => {
    process.env.GITEA_TOKEN = 'mytoken';
    process.env.GITEA_USERNAME = 'myuser';
  ...
  it('does not inject credentials when only GITEA_TOKEN is set (username missing)', () => {
    process.env.GITEA_TOKEN = 'mytoken';
    const p = makeProvider();
    const url = 'https://gitea.com/owner/repo.git';
    expect(p.resolvePushUrl(url, FAKE_ROOT)).toBe(url);
  ...
  it('does not inject credentials when only GITEA_USERNAME is set (token missing)', () => {
    process.env.GITEA_USERNAME = 'myuser';
    const p = makeProvider();
    const url = 'https://gitea.com/owner/repo.git';
    expect(p.resolvePushUrl(url, FAKE_ROOT)).toBe(url);
  ...
  it('expands an owner/repo slug to a full gitea.com URL when GITEA_URL is not set', () => {
    const p = makeProvider();
    const result = p.resolvePushUrl('owner/repo', FAKE_ROOT);
    expect(result).toBe('https://gitea.com/owner/repo.git');
  });

  it('expands an owner/repo.git slug (with .git suffix) correctly', () => {
    const p = makeProvider();
    const result = p.resolvePushUrl('owner/repo.git', FAKE_ROOT);
    expect(result).toBe('https://gitea.com/owner/repo.git');
  });

  it('expands a slug with a dotted repo name (owner/my.repo)', () => {
    const p = makeProvider();
    const result = p.resolvePushUrl('owner/my.repo', FAKE_ROOT);
    expect(result).toBe('https://gitea.com/owner/my.repo.git');
  });

  it('expands an owner/repo slug using GITEA_URL when set', () => {
    process.env.GITEA_URL = 'https://gitea.mycompany.com';
    const p = makeProvider();
    const result = p.resolvePushUrl('owner/repo', FAKE_ROOT);
  ...
  it('strips trailing slash from GITEA_URL before expanding a slug', () => {
    process.env.GITEA_URL = 'https://gitea.mycompany.com/';
    const p = makeProvider();
    const result = p.resolvePushUrl('owner/repo', FAKE_ROOT);
    expect(result).toBe('https://gitea.mycompany.com/owner/repo.git');
...
describe('GiteaProvider.extractRepoSlug', () => {
  ...
  it('extracts host|owner/repo from a gitea.com HTTPS URL', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('https://gitea.com/owner/repo.git', FAKE_ROOT)).toBe(
      'https://gitea.com|owner/repo',
    );
  });

  it('extracts host|owner/repo from an HTTPS URL without .git suffix', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('https://gitea.com/owner/repo', FAKE_ROOT)).toBe(
      'https://gitea.com|owner/repo',
    );
  });

  it('extracts host|owner/repo from a self-hosted HTTPS URL', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('https://gitea.mycompany.com/owner/repo.git', FAKE_ROOT)).toBe(
      'https://gitea.mycompany.com|owner/repo',
    );
  });

  it('extracts host|owner/repo from a git@ SSH SCP URL', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('git@gitea.com:owner/repo.git', FAKE_ROOT)).toBe(
      'https://gitea.com|owner/repo',
    );
  });

  it('extracts host|owner/repo from an ssh:// URL with git@ user', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('ssh://git@gitea.com/owner/repo.git', FAKE_ROOT)).toBe(
      'https://gitea.com|owner/repo',
    );
  });

  it('extracts host|owner/repo from an ssh:// URL without explicit user', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('ssh://gitea.com/owner/repo.git', FAKE_ROOT)).toBe(
      'https://gitea.com|owner/repo',
    );
  });

  it('strips embedded userinfo from an HTTPS URL when extracting slug', () => {
    const p = makeProvider();
    // userinfo should not leak into the host stored in the opaque slug
    expect(p.extractRepoSlug('https://myuser:mytoken@gitea.com/owner/repo.git', FAKE_ROOT)).toBe(
      'https://gitea.com|owner/repo',
    );
  });

  it('extracts slug correctly for a dotted repo name (owner/my.repo)', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('owner/my.repo', FAKE_ROOT)).toBe('https://gitea.com|owner/my.repo');
  });

  it('extracts slug from an owner/repo.git slug shorthand', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('owner/repo.git', FAKE_ROOT)).toBe('https://gitea.com|owner/repo');
  });

  it('expands an owner/repo slug using gitea.com when GITEA_URL is not set', () => {
    const p = makeProvider();
    expect(p.extractRepoSlug('owner/repo', FAKE_ROOT)).toBe('https://gitea.com|owner/repo');
  });

  it('expands an owner/repo slug using GITEA_URL when set', () => {
    process.env.GITEA_URL = 'https://gitea.mycompany.com';
    const p = makeProvider();
    expect(p.extractRepoSlug('owner/repo', FAKE_ROOT)).toBe(
      'https://gitea.mycompany.com|owner/repo',
    );
  });
  ...
  it('throws when GITEA_URL contains a pipe character', () => {
    process.env.GITEA_URL = 'https://gitea|bad.com';
    const p = makeProvider();
    expect(() => p.extractRepoSlug('owner/repo', FAKE_ROOT)).toThrow(/GITEA_URL must not contain/);
  });

  it('throws when the URL cannot be parsed as a Gitea URL', () => {
    const p = makeProvider();
    // An HTTPS URL that cannot be reduced to owner/repo (extra path segment)
    expect(() => p.extractRepoSlug('https://gitea.com/group/subgroup/repo.git', FAKE_ROOT)).toThrow(
