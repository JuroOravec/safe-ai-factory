...
set -eu
cd /workspace
echo "[sandbox/python-conda-node/startup] Installing dependencies (conda)..."
if [ -f environment.yml ]; then
  conda env update -n base -f environment.yml --prune
elif [ -f requirements.txt ]; then
  pip install -r requirements.txt
else
  echo "[sandbox/python-conda-node/startup] No environment.yml or requirements.txt found — skipping."
fi
echo "[sandbox/python-conda-node/startup] Done."
