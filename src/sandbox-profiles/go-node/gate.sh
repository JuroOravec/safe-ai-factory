...
echo "[sandbox/go-node/gate] WARNING: Running DEFAULT gate (go vet + go test)."
echo "[sandbox/go-node/gate] Define a custom --gate-script with more checks for better results."

echo "[sandbox/go-node/gate] Running go vet..."
go vet ./...
echo "[sandbox/go-node/gate] Running go test..."
go test ./...
echo "[sandbox/go-node/gate] Gate PASSED."
