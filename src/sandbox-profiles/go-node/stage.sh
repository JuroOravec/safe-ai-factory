...
if [ -f Procfile ] && grep -q '^web:' Procfile; then
  echo "[sandbox/go-node/stage] Starting web server (Procfile web)..."
  exec sh -c "$(grep '^web:' Procfile | sed 's/^web: //')"
elif [ -f main.go ]; then
  echo "[sandbox/go-node/stage] Building and starting app (main.go)..."
  go build -o /tmp/app . && exec /tmp/app
elif [ -d cmd ]; then
  echo "[sandbox/go-node/stage] Building and starting app (cmd/)..."
  go build -o /tmp/app ./cmd/... && exec /tmp/app
else
  echo "[sandbox/go-node/stage] No entrypoint found — sidecar is the only process."
  wait
fi
