...
if [ -f Procfile ] && grep -q '^web:' Procfile; then
  echo "[sandbox/python-poetry/stage] Starting web server (Procfile web)..."
  exec sh -c "$(grep '^web:' Procfile | sed 's/^web: //')"
elif [ -f app.py ]; then
  echo "[sandbox/python-poetry/stage] Starting web server (app.py)..."
  if [ -f pyproject.toml ]; then exec poetry run python app.py; else exec python app.py; fi
elif [ -f main.py ]; then
  echo "[sandbox/python-poetry/stage] Starting web server (main.py)..."
  if [ -f pyproject.toml ]; then exec poetry run python main.py; else exec python main.py; fi
else
  echo "[sandbox/python-poetry/stage] No entrypoint found — sidecar is the only process."
  wait
fi
