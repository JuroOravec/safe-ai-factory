...
if [ -f Procfile ] && grep -q '^web:' Procfile; then
  echo "[sandbox/rust-python/stage] Starting web server (Procfile web)..."
  exec sh -c "$(grep '^web:' Procfile | sed 's/^web: //')"
elif [ -f Cargo.toml ]; then
  echo "[sandbox/rust-python/stage] Building and starting app (cargo run --release)..."
  exec cargo run --release
else
  echo "[sandbox/rust-python/stage] No entrypoint found — sidecar is the only process."
  wait
fi
